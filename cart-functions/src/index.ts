...
export const addItem = onRequest({
  ...
  try {
    ...
    const { cartId, sku, qty, priceAtAdd, name, brand, model, imageUrl, inStock } = req.body;
    
    if (!cartId || !sku) {
      res.status(400).json({ ok: false, error: 'Cart ID and SKU are required' });
      return;
    }
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (!cartDoc.exists) {
        ...
        const existingItemIndex = existingItems.findIndex((item: any) => item.sku === sku);
...
export const removeItem = onRequest({
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const cartData = cartDoc.data();
      const existingItems = cartData?.items || [];
      const filteredItems = existingItems.filter((item: any) => item.sku !== sku);
...
export const mergeCartOnSignIn = onRequest({
  ...
  try {
    ...
    if (!cartId || !userId) {
      res.status(400).json({ ok: false, error: 'Cart ID and User ID are required' });
      return;
    }
    ...
    if (!userCartDoc.exists) {
      ...
      guestItems.forEach((guestItem: any) => {
        const existingItemIndex = mergedItems.findIndex((item: any) => item.sku === guestItem.sku);
