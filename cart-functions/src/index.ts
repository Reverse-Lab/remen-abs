...
// 회원 장바구니에 아이템 추가
export const addUserItem = onRequest({
  region: "asia-northeast3",
}, async (req, res) => {
  try {
    // CORS 설정
    ...
    const {userId, sku, qty, priceAtAdd, name, brand, model, imageUrl, inStock} = req.body;
    
    if (!userId || !sku) {
      res.status(400).json({ok: false, error: "User ID and SKU are required"});
      return;
    }

    logger.info("Adding item to user cart", {userId, sku, qty});
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (!userCartDoc.exists) {
        ...
        transaction.set(userCartRef, newUserCart);
      } else {
        // 기존 회원 장바구니에 아이템 추가/업데이트
        const userCartData = userCartDoc.data();
        const existingItems = userCartData?.items || [];
        const existingItemIndex = existingItems.findIndex(
          (item: any) => item.sku === sku,
        );
    ...
    res.json({ok: true, message: "Item added to user cart successfully"});
  } catch (error) {
